...
export function URLUploadButton() {
  ...
  const ingest = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (url != DEFAULT_URL_TEXT) {
      formData.append("url", url);
    }

    const response = await fetch("/api/retrieval/url_inges", {
      method: "POST",
      body: formData
    });
  ...
  return (
    ...
    <textarea
      ...
      rows={1}
      value={url}
      placeholder='Insert URL (ex: https://google.com)'
      onChange={(e) => setURL(e.target.value)}
    ></textarea>
    <button type="submit" className="shrink-0 px-8 py-2 bg-[#fb442c] rounded w-28" disabled={url == DEFAULT_URL_TEXT || !url}>
